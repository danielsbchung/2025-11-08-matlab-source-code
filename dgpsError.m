...
%% DGPS 측위 + DOP 계산
 ...
 for ti = 1: n_epoch
     ...
     while(dx > 10^-4 && iter < max_iter)
         H_standalone = zeros(n_vis,4); % H 행렬 이름은 편의상 유지
...
errors_dgps_enu = user.pos_dgps_enu(analysis_range,:) - true.enu(analysis_range,:);
rms_dgps_e = sqrt(mean(errors_dgps_enu(:,1).^2));
rms_dgps_n = sqrt(mean(errors_dgps_enu(:,2).^2));
rms_dgps_u = sqrt(mean(errors_dgps_enu(:,3).^2));
rms_dgps_3d = sqrt(mean(sum(errors_dgps_enu.^2, 2)));

fprintf('\n--- [DGPS] RMS 오차 분석 결과 (%.0f초 이후) ---\n', ini_epoch);
...
%% --- 2. DOP 그래프 출력 (DGPS) ---
figure('Name', 'DOP (DGPS)');
plot(elapsedTime, DOP_values_dgps);
grid on;
title('DOP values over Time (DGPS)');
