...
% --- 0.1. 기본 설정 ---
n_epoch     = size(user.GPSTime,1);     
maskangle   = 15;                       % 차폐각 (deg)
nt          = 32;                       % GPS 위성 개수
ini_epoch   = 5*60;                     % 미지정수 계산용 초기 정지시간 (sec)
elapsedTime = user.GPSTime - user.GPSTime(1);
...
%% 2. CDGPS 측위 (요구 조건 C: user_xyz0 사용)
% =========================================================================
...
% 1.2 [요구 조건 C 수정] "참값(true.xyz)" 대신 "제공된 초기 위치(user_xyz0)" 사용
fprintf('   (Puzzle 모드: 조교가 제공한 user_xyz0를 초기 위치로 사용합니다.)\n');

N_estimates = zeros(ini_epoch, n_other_sats);
for ti = 1:ini_epoch
    ref_sat_pos = user.svpos(ti, 3*ref_sat_prn-2:3*ref_sat_prn); 
    
    % [수정됨] true.xyz 대신 user_xyz0 사용
    % user_xyz0는 epoch마다 변하지 않는 고정된 값입니다.
    true_pos_ti = user_xyz0; 
...
%% --- 4. "Puzzle" 궤적 그래프 (NE, UpTime) ---
figure('Name', '실험 3: CDGPS 궤적 (글자 찾기)');
hold on;
% 초기 정지 구간(ini_epoch)은 제외하고 글씨 쓴 부분만 플롯
plot(user.pos_cdgps_enu(ini_epoch+1:end, 1), user.pos_cdgps_enu(ini_epoch+1:end, 2), '.g');
grid on; axis equal;
title('CDGPS (North-East) 궤적 - Puzzle');
xlabel('East (m)'); ylabel('North (m)');
hold off;

figure('Name', '실험 3: CDGPS 궤적 (Up vs Time)');
hold on;
plot(elapsedTime, user.pos_cdgps_enu(:,3), '.g');
grid on;
title('CDGPS (Up Trajectory)');
